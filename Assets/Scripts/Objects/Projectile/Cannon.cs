...
public class Cannon : MonoBehaviour
{
     ...
     public void Initialize(AttackData attackData, float towerDamage, Vector3 aimPoint, LayerMask mask)
     {
         damage           = towerDamage;
         speed            = attackData.projectileSpeed;
         areaEffectPrefab = attackData.areaEffectPrefab;
         areaRadius       = attackData.areaRadius;
         enemyLayerMask   = mask;
         targetPosition   = aimPoint;

         rb = GetComponent<Rigidbody>();
         // 포물선 초기 속도 계산
         Vector3 velocity = CalculateBallisticVelocity(transform.position, targetPosition, speed);
